...
多模态嵌入是将**不同类型的数据银蛇到同一个共享的向量空间**，在这个统一的向量空间里，描述同一类物品的不同类型数据会比较接近





在图文多模态领域，CLIP架构清晰简介，它采用双编码器结构，包含一个图像编码器和一个文本编码器，分别将图像和文本映射到同一个共享的向量空间中。在训练时采用**对比学习**策略，在处理一批图文数据时，模型的目标时：最大化正确图片对的向量相似度，最小化所有错误匹配的相似度。CLIP赋予了模型**零样本识别能力**（没见过某一类别或任务的训练样本情况下，仍能完成任务）
...
+ **倒数排序融合RRF**，关注每个文档子啊各自结果集中的排名，核心思想是：**一个文档在不同检索系统中的排名越靠前，它的最终得分就越高**。
