...
`Policy Gradient(on policy)`学习函数**$\pi(a \mid s; \theta)$**，策略执行的时候根据$\pi(a|s)$抽样一个动作a
...
$$
更新策略$\theta_{t+1} = \theta_t + \beta \cdot \frac{\partial V(s;\theta)}{\partial \theta}$
...
$$
对于**离散动作**，由于动作有限，策略梯度的函数可以求导化简，而Q值通过MC估计得到，能够直接计算
...
\frac{\partial V(s;\theta)}{\partial \theta}
= \sum_{a} \frac{\partial \pi(a|s;\theta)}{\partial \theta} \cdot Q_{\pi}(s,a)
= \sum_{a} \pi(a|s;\theta) \cdot \frac{\partial \log\pi(a|s;\theta)}{\partial \theta} \cdot Q_{\pi}(s,a)
= \mathbb{E}_{A}\left[ \frac{\partial \log\pi(A|s;\theta)}{\partial \theta} \cdot Q_{\pi}(s,A) \right]
$$


根据$\pi{(\cdot|s;\theta)}$做一次抽样，得到某个动作$\hat a$，计算$$ \frac{\partial \log\pi(\hat{a}|s;\theta)}{\partial \theta} \cdot Q_{\pi}(s,\hat{a}),\quad \text{记作}g(\hat{a},\theta) $$，做`蒙特卡洛近似`，Q值可以通过**MC**计算
...
`PPO`是`PG`算法的一种改进，PG算法最u大的去恶电视参数更新慢，没更新一次参数都需要重新采样，为了提升训练速度，最直观的方法是让采样到的数据可以重复使用，为此，可以将训练数据通过另一个相同结构的网络得到。
